    ...
    uint signal_size)
{
  ...
  at::Half *b_imag = &a_real[4 * N + N_1];
  at::Half *b_real_2 = &a_real[4 * N + 2 * N_1];
  at::Half *b_imag_2 = &a_real[4 * N + 3 * N_1];
    ...
    for (int b_tile_id = 0; b_tile_id < B_TILE_SIZE; b_tile_id++)
    {
      ...
      for (int i = 0; i < items_per_thread_input / 2; i++)
      {
        ...
        if(in_gate != nullptr){
          ...
          reinterpret_cast<__half2 *>(a_real_2)[a_idx] = reinterpret_cast<__half2 *>(x_input_data)[i];
        }
      }
      ...
      for (int k_idx = 0; k_idx < 8 / WARP_TILE_SIZE; k_idx++)
      {
        ...
        // outer DFT(x)
        complex_matmul_r2c_256<wmma::col_major, wmma::row_major, true, true, MATMUL_WARP_WIDTH_1, false, true>(
            reinterpret_cast<half *>(a_real_2 + k_idx_offset), // read from SRAM
