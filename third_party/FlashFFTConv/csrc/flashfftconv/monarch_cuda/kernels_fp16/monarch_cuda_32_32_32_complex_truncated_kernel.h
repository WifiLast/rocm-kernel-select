...
#include "monarch_cuda_shared_truncated.h"
#if defined(__HIP_PLATFORM_AMD__)
...
__global__ void monarch_conv_cuda_32_32_32_complex_kernel_truncated(
    ...
    uint kernel_trunc)
{
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
    {
      ...
      wmma::load_matrix_sync(a_frag_dft_N_1[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + a_idx), sqrt_N_1);
      ...
      wmma::load_matrix_sync(a_frag_dft_N_1[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + a_idx), sqrt_N_1);
  ...
  for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_1024_dft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real) + k_idx_offset + b_idx), sqrt_N_1);
        wmma::load_matrix_sync(twiddle_1024_dft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag) + k_idx_offset + b_idx), sqrt_N_1);
      }
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_1 + k * WMMA_K : k * WMMA_K * sqrt_N_1 + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_32_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + b_idx), sqrt_N_1);
      wmma::load_matrix_sync(twiddle_32_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + b_idx), sqrt_N_1);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_1 + k * WMMA_K : k * WMMA_K * sqrt_N_1 + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_32_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real_2) + b_idx), sqrt_N_1);
      wmma::load_matrix_sync(twiddle_32_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag_2) + b_idx), sqrt_N_1);
    }
  }
  ...
  for (int h_tile_id = 0; h_tile_id < H_TILE_SIZE; h_tile_id++)
  {
    ...
    for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
    {
      ...
      for (int j_a = 0; j_a < MATMUL_WARP_WIDTH_1; j_a++)
      {
        ...
        for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
        {
          ...
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real + a_idx)), sqrt_N_1);
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag + a_idx)), sqrt_N_1);
        }
      }
    }
    ...
    for (int b_tile_id = 0; b_tile_id < B_TILE_SIZE; b_tile_id++)
    {
      ...
      // 1024 / 32 = 32
      for (int k_idx = 0; k_idx < (32 - kernel_trunc) / WARP_TILE_SIZE; k_idx++)
      {
      ...
      for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
      {
        ...
        complex_matmul_load_b_truncated<wmma::col_major, wmma::row_major, false, false, MATMUL_WARP_WIDTH_1, false, true>(
            reinterpret_cast<half *>(a_real + k_idx_offset), // this is the output
            reinterpret_cast<half *>(a_imag + k_idx_offset), // this is the output
            sqrt_N_1,
            N,
        ...
        complex_matmul_truncated<wmma::row_major, wmma::row_major, false, false, MATMUL_WARP_WIDTH_1, true, false>(
        ...
        complex_matmul_truncated<wmma::row_major, wmma::row_major, false, true, MATMUL_WARP_WIDTH_1, true, true>(
            reinterpret_cast<half *>(a_real + k_idx_offset),
            reinterpret_cast<half *>(a_imag + k_idx_offset),
            sqrt_N_1,
            N,
        ...
        complex_matmul_truncated<wmma::row_major, wmma::row_major, false, true, MATMUL_WARP_WIDTH_1, false, true>(
            reinterpret_cast<half *>(a_real + k_idx_offset),
            reinterpret_cast<half *>(a_imag + k_idx_offset),
