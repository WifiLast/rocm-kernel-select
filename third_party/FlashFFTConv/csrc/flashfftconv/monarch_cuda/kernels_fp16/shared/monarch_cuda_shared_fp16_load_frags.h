  ...
  if (a_frag_from_acc) {
    ...
    for (int j_a = 0; j_a < MATMUL_WARP_WIDTH; j_a++) {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH; k++) {
        ...
        wmma::load_matrix_sync(a_frag[j_a][k][0],reinterpret_cast<const wmma_half_t*>( a_real + a_idx), 256);
        wmma::load_matrix_sync(a_frag[j_a][k][1],reinterpret_cast<const wmma_half_t*>( a_imag + a_idx), 256);
      }
    }  
  }
}
  ...
  if (a_frag_from_acc) {
    ...
    for (int j_a = 0; j_a < MATMUL_WARP_WIDTH; j_a++) {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH; k++) {
        ...
        wmma::load_matrix_sync(a_frag[j_a][k][1],reinterpret_cast<const wmma_half_t*>( a_imag + a_idx), 1024);
      }
    }  
  }
}
