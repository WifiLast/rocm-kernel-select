    ...
    uint signal_size)
{
  ...
  for (int k_idx = 0; k_idx < 16 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_1024_dft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real) + k_idx_offset + b_idx), sqrt_N_2);
        wmma::load_matrix_sync(twiddle_1024_dft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag) + k_idx_offset + b_idx), sqrt_N_2);
      }
    }
  }
  ...
  for (int k_idx = 0; k_idx < 64 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_1024_idft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real) + k_idx_offset + b_idx), 1024);
        wmma::load_matrix_sync(twiddle_1024_idft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag) + k_idx_offset + b_idx), 1024);
      }
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
    {
      ...
      wmma::load_matrix_sync(twiddle_32_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(twiddle_32_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_2 + k * WMMA_K : k * WMMA_K * sqrt_N_2 + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_32_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real_2) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(twiddle_32_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag_2) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int h_tile_id = 0; h_tile_id < H_TILE_SIZE; h_tile_id++)
  {
    ...
    for (int b_tile_id = 0; b_tile_id < B_TILE_SIZE; b_tile_id++)
    {
      ...
      for (int k_idx = 0; k_idx < 64 / WARP_TILE_SIZE; k_idx++)
      {
        ...
        // outer DFT(x)
        complex_matmul_r2c_1024<wmma::col_major, wmma::row_major, true, true, MATMUL_WARP_WIDTH_1, false, true>(
            reinterpret_cast<half *>(a_real_2 + k_idx_offset), // read from HBM
