    ...
    uint sqrt_N)
{
  ...
  // for the dft
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> b_frag_dft[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  // for the idft
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> b_frag_idft[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  // for the dft
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::col_major> a_frag_dft[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  // for twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_16_dft_frag[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  // for twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_16_idft_frag[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];

  // for 256 twiddle
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_256_dft_frag[16 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  // for 256 idft twiddle
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::col_major> twiddle_256_idft_frag[16 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  ...
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> k_frag[16 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  ...
  bool a_trans = true;
  bool b_trans = false;
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_K, WMMA_N, wmma_half_t> acc_frag_1[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
...
#pragma unroll
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
    {
      ...
      wmma::load_matrix_sync(a_frag_dft[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + a_idx), sqrt_N);
      wmma::load_matrix_sync(b_frag_dft[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + b_idx), sqrt_N);
      wmma::load_matrix_sync(a_frag_dft[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + a_idx), sqrt_N);
      wmma::load_matrix_sync(b_frag_dft[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + b_idx), sqrt_N);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(b_frag_idft[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real_2) + b_idx), sqrt_N);
      wmma::load_matrix_sync(b_frag_idft[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag_2) + b_idx), sqrt_N);
    }
  }
  ...
  for (int k_idx = 0; k_idx < 16 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_256_dft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real) + k_idx_offset + b_idx), sqrt_N);
        wmma::load_matrix_sync(twiddle_256_dft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag) + k_idx_offset + b_idx), sqrt_N);
      }
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_16_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + b_idx), sqrt_N);
      wmma::load_matrix_sync(twiddle_16_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + b_idx), sqrt_N);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_16_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real_2) + b_idx), sqrt_N);
      wmma::load_matrix_sync(twiddle_16_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag_2) + b_idx), sqrt_N);
    }
  }
    ...
    for (int k_idx = 0; k_idx < 16 / WARP_TILE_SIZE; k_idx++)
    {
      ...
      for (int j_a = 0; j_a < MATMUL_WARP_WIDTH; j_a++)
      {
        ...
        for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
        {
          ...
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real + a_idx)), sqrt_N);
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag + a_idx)), sqrt_N);
        }
      }
    }
    ...
    for (int b_tile_id = 0; b_tile_id < B_TILE_SIZE; b_tile_id++)
    {
      ...
      for (int i = 0; i < items_per_thread_input / 2; i++)
      {
        ...
        reinterpret_cast<__half2 *>(a_real)[a_idx] = __half2(
          __float2half(__bfloat162float(reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i]) / ADJUST_FACTOR),
          __float2half(__bfloat162float(reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i + 1]) / ADJUST_FACTOR)
        );
      }
      ...
      for (int i = 0; i < items_per_thread_input / 2; i++)
      {
        ...
        reinterpret_cast<__half2 *>(a_real_2)[a_idx] = __half2(
          __float2half(__bfloat162float(reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i])),
          __float2half(__bfloat162float(reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i + 1]))
        );
      }
      ...
      for (int k_idx = 0; k_idx < 16 / WARP_TILE_SIZE; k_idx++)
      {
        ...
        // if (threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x == 0 && blockIdx.y == 0) {
        //   printf("Values in a_real, a_imag before mul\n");
        //   for (int i = 0; i < items_per_thread_input; i++) {
        //       a_idx = i * num_threads + thread_id;
        //       printf("%f + %fi, ", __half2float(a_real[a_idx]), __half2float(a_imag[a_idx]));
        //   }
        ...
        // if (threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x == 0 && blockIdx.y == 0) {
        //   printf("Values in a_real, a_imag\n");
        //   for (int i = 0; i < items_per_thread_input; i++) {
        //       a_idx = i * num_threads + thread_id;
        //       printf("%f + %fi, ", __half2float(a_real[a_idx]), __half2float(a_imag[a_idx]));
        //   }
      ...
      for (int i = 0; i < items_per_thread_input / 2; i++)
      {
        ...
        reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i] = __float2bfloat16(__half2float(scratch.x));
        reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i + 1] = __float2bfloat16(__half2float(scratch.y));
      }
...
#pragma unroll
      ...
      {
        a_idx = i * num_threads + thread_id;
        real = __hmul2(reinterpret_cast<__half2 *>(a_real_2)[a_idx],
        __half2(__float2half(float(N) * ADJUST_FACTOR), __float2half(float(N) * ADJUST_FACTOR)));
        imag = __hmul2(reinterpret_cast<__half2 *>(a_imag_2)[a_idx],
        __half2(__float2half(float(N) * ADJUST_FACTOR), __float2half(float(N) * ADJUST_FACTOR)));
    ...
    // for(int i = 0; i < items_per_thread_input; i++) {
    //     reinterpret_cast<__half2 *>(temp)[i] = __hmul2(reinterpret_cast<__half2 *>(temp)[i], __half2(__float2half(float(N)), __float2half(float(N))));
    // }
