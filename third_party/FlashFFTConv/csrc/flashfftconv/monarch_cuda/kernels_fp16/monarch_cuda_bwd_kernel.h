    ...
    uint sqrt_N)
{
  ...
  // for kernels
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> k_frag[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  // for twiddles
...
#pragma unroll
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + b_idx), sqrt_N);
      wmma::load_matrix_sync(twiddle_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + b_idx), sqrt_N);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real_2) + b_idx), sqrt_N);
      wmma::load_matrix_sync(twiddle_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag_2) + b_idx), sqrt_N);
    }
  }
  ...
  for (int h_tile_id = 0; h_tile_id < H_TILE_SIZE; h_tile_id++)
  {
    ...
    for (int i = 0; i < items_per_thread_input / 2; i++)
    {
      ...
      reinterpret_cast<__half2 *>(a_real)[a_idx] = __half2(a_input_data[2 * i].real(), a_input_data[2 * i + 1].real());

      reinterpret_cast<__half2 *>(a_imag)[a_idx] = __half2(a_input_data[2 * i].imag(), a_input_data[2 * i + 1].imag());
    }
    ...
    for (int j_a = 0; j_a < MATMUL_WARP_WIDTH; j_a++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
      {
        ...
        wmma::load_matrix_sync(k_frag[j_a][k][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real + a_idx)), sqrt_N);
        wmma::load_matrix_sync(k_frag[j_a][k][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag + a_idx)), sqrt_N);
      }
    }
  ...
  __syncthreads();
    ...
    for (int b_tile_id = 0; b_tile_id < B_TILE_SIZE; b_tile_id++)
    {
      ...
      complex_matmul_r2c_load_b<wmma::col_major, wmma::row_major, false, false, MATMUL_WARP_WIDTH, false, false>(
          reinterpret_cast<half *>(a_real), // read from SRAM
      ...
      // first DFT(x)
      complex_matmul_r2c_load_b<wmma::col_major, wmma::row_major, false, false, MATMUL_WARP_WIDTH, false, false>(
          reinterpret_cast<half *>(a_real_2), // read from HBM
      ...
      {
        a_idx = i * num_threads + thread_id;
        reinterpret_cast<__half2 *>(b_real_2)[a_idx] = __hmul2(
            reinterpret_cast<__half2 *>(a_real_2)[a_idx],
            __half2(__float2half(float(N)), __float2half(float(N))));
        reinterpret_cast<__half2 *>(b_imag_2)[a_idx] = __hmul2(
      ...
      for (int j_a = 0; j_a < MATMUL_WARP_WIDTH; j_a++){
        ...
        for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
        {
          for(int i=0; i < k_frag[j_a][k][1].num_elements; i++){
            k_frag[j_a][k][1].x[i] = __hneg(k_frag[j_a][k][1].x[i]);
          }
        }
      }
      ...
      if(out_gate != nullptr){
        for (int i = 0; i < items_per_thread_input / 2; i++)
        {
            ...
            reinterpret_cast<__half2 *>(dgate_data)[i] = reinterpret_cast<__half2 *>(a_real_2)[a_idx];
        ...
        BlockStore_Sequence().Store(
          reinterpret_cast<float *>(dout_gate + input_offset),
      ...
      for (int i = 0; i < items_per_thread_input / 2; i++)
      {
        ...
        complex_mul_conj_half2(
            ...
            reinterpret_cast<__half2 *>(b_real_2)[a_idx],
            reinterpret_cast<__half2 *>(b_imag_2)[a_idx],
