...
using wmma_half_t = rocwmma::float16_t;
#else
using namespace nvcuda;
using wmma_half_t = half;
#endif
...
__global__ void monarch_conv_cuda_kernel(
    ...
    const c10::complex<at::Half> *__restrict__ b_16,                        // 16 x 16
    const c10::complex<at::Half> *__restrict__ twiddle_factors_N_fft,  // 8192
    ...
    const c10::complex<at::Half> *__restrict__ b_16_ifft,                   // 16 x 16
    const c10::complex<at::Half> *__restrict__ twiddle_factors_N_ifft, // 8192
    ...
    uint signal_size)
{
  ...
  at::Half *b_imag = &a_real[2 * N + N_1];
  at::Half *b_real_2 = &a_real[2 * N + 2 * N_1];
  at::Half *b_imag_2 = &a_real[2 * N + 3 * N_1];
  ...
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_K, WMMA_N, wmma_half_t> acc_frag_1[MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_K, WMMA_N, wmma_half_t> acc_frag_2[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
    {
      ...
      wmma::load_matrix_sync(b_frag_dft_N_1[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + b_idx), sqrt_N_1);
      wmma::load_matrix_sync(b_frag_dft_N_1[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + b_idx), sqrt_N_1);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_1 + k * WMMA_K : k * WMMA_K * sqrt_N_1 + j_b * WMMA_N;
      wmma::load_matrix_sync(b_frag_idft_N_1[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real_2) + b_idx), sqrt_N_1);
      wmma::load_matrix_sync(b_frag_idft_N_1[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag_2) + b_idx), sqrt_N_1);
    }
  }
  ...
  for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_256_dft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real) + k_idx_offset + b_idx), sqrt_N_2);
        wmma::load_matrix_sync(twiddle_256_dft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag) + k_idx_offset + b_idx), sqrt_N_2);
      }
    }
  }
  ...
  for (int k_idx = 0; k_idx < 8 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_256_idft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real) + k_idx_offset + b_idx), 256);
        wmma::load_matrix_sync(twiddle_256_idft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag) + k_idx_offset + b_idx), 256);
      }
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
    {
      ...
      wmma::load_matrix_sync(twiddle_16_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(twiddle_16_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_2 + k * WMMA_K : k * WMMA_K * sqrt_N_2 + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_16_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real_2) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(twiddle_16_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag_2) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
    {
      ...
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_2 + k * WMMA_K : k * WMMA_K * sqrt_N_2 + j_b * WMMA_N;
      wmma::load_matrix_sync(a_frag_dft_N_2[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + a_idx), sqrt_N_2);
      wmma::load_matrix_sync(b_frag_dft_N_2[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(a_frag_dft_N_2[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + a_idx), sqrt_N_2);
      wmma::load_matrix_sync(b_frag_dft_N_2[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_2 + k * WMMA_K : k * WMMA_K * sqrt_N_2 + j_b * WMMA_N;
      wmma::load_matrix_sync(b_frag_idft_N_2[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_real_2) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(b_frag_idft_N_2[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(b_imag_2) + b_idx), sqrt_N_2);
    }
  }
    ...
    for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
    {
      ...
      for (int j_a = 0; j_a < MATMUL_WARP_WIDTH_2; j_a++)
      {
        ...
        for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
        {
          ...
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_real + a_idx)), sqrt_N_2);
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<half *>(a_imag + a_idx)), sqrt_N_2);
        }
      }
    }
    ...
    for (int b_tile_id = 0; b_tile_id < B_TILE_SIZE; b_tile_id++)
    {
      ...
      for (int i = 0; i < items_per_thread_input / 2; i++)
      {
        ...
        reinterpret_cast<__half2 *>(a_real)[a_idx] = __half2(
          __float2half(__bfloat162float(reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i]) / N),
          __float2half(__bfloat162float(reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i + 1]) / N)
        );
      }
      ...
      for (int k_idx = 0; k_idx < 8 / WARP_TILE_SIZE; k_idx++)
      {
        ...
        complex_matmul_r2c_256<wmma::col_major, wmma::row_major, true, true, MATMUL_WARP_WIDTH_1, false, true>(
            reinterpret_cast<half *>(a_real + k_idx_offset), // read from HBM
      ...
      for (int i = 0; i < items_per_thread_input / 2; i++)
      {
        ...
        reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i] = __float2bfloat16(__half2float(scratch.x) * N);
        reinterpret_cast<__nv_bfloat16 *>(x_input_data)[2 * i + 1] = __float2bfloat16(__half2float(scratch.y) * N);
      }
