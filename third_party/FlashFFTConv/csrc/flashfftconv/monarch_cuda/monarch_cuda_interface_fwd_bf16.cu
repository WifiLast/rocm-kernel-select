...
#include "kernels_fp16/monarch_cuda_16_16_16_kernel_fp16_bf16_inp.h"
#include "kernels_bf16/monarch_cuda_16_16_16_kernel_bf16.h"
#include "kernels_fp16/monarch_cuda_32_16_16_kernel_fp16_bf16_inp.h"
#include "kernels_bf16/monarch_cuda_32_16_16_kernel_bf16.h"
#include "kernels_bf16/monarch_cuda_16_32_32_kernel_bf16.h"
#include "kernels_bf16/monarch_cuda_32_32_32_kernel_bf16.h"
#if defined(__HIP_PLATFORM_AMD__)
...
#define CHECK_LAST_CUDA_ERROR() checkLastBF16Fwd(__FILE__, __LINE__)
void checkLastBF16Fwd(const char* const file, const int line)
{
  ...
  switch (fftsize) {
    ...
    case 1024:
      if (B >= 8 && (B % 8) == 0 && H >= 8 && (H % 8) == 0) {
        ...
        monarch_conv_cuda_kernel<32, 1, 1024, 2, false, 8, 8><<<gridDim, blockDim, 6 * fftsize * sizeof(half) + 2 * fftsize * sizeof(float)>>>(
          static_cast<at::BFloat16 *>(x.data_ptr()),
          in_gate.has_value() ? static_cast<at::BFloat16 *>(in_gate.value().data_ptr()) : nullptr,
          static_cast<complex_bfloat16_t *>(k_f.data_ptr()),
          static_cast<complex_bfloat16_t *>(f_sqrt_N_fft.data_ptr()),
        ...
        monarch_conv_cuda_kernel<32, 1, 1024, 2, false, 4, 8><<<gridDim, blockDim, 6 * fftsize * sizeof(half) + 2 * fftsize * sizeof(float)>>>(
          static_cast<at::BFloat16 *>(x.data_ptr()),
          in_gate.has_value() ? static_cast<at::BFloat16 *>(in_gate.value().data_ptr()) : nullptr,
          static_cast<complex_bfloat16_t *>(k_f.data_ptr()),
          static_cast<complex_bfloat16_t *>(f_sqrt_N_fft.data_ptr()),
        ...
        monarch_conv_cuda_kernel<32, 1, 1024, 2, false, 1, 8><<<gridDim, blockDim, 6 * fftsize * sizeof(half) + 2 * fftsize * sizeof(float)>>>(
          static_cast<at::BFloat16 *>(x.data_ptr()),
          in_gate.has_value() ? static_cast<at::BFloat16 *>(in_gate.value().data_ptr()) : nullptr,
          static_cast<complex_bfloat16_t *>(k_f.data_ptr()),
          static_cast<complex_bfloat16_t *>(f_sqrt_N_fft.data_ptr()),
        ...
        monarch_conv_cuda_kernel<32, 1, 1024, 2, false, 1, 1><<<gridDim, blockDim, 6 * fftsize * sizeof(half) + 2 * fftsize * sizeof(float)>>>(
          static_cast<at::BFloat16 *>(x.data_ptr()),
          in_gate.has_value() ? static_cast<at::BFloat16 *>(in_gate.value().data_ptr()) : nullptr,
          static_cast<complex_bfloat16_t *>(k_f.data_ptr()),
          static_cast<complex_bfloat16_t *>(f_sqrt_N_fft.data_ptr()),
  ...
  switch (fftsize) {
    case 8192:
      if (B >= 8 && (B % 8) == 0 && (H % 8) == 0) {
        ...
        monarch_conv_cuda_kernel<32, 8, 8192, 2, 1, 16, false, 4, 8, 8><<<gridDim, blockDim, (2 * fftsize + 4 * 1024) * sizeof(half)>>>(
            static_cast<at::BFloat16 *>(x.data_ptr()),
          in_gate.has_value() ? static_cast<at::BFloat16 *>(in_gate.value().data_ptr()) : nullptr,
            static_cast<complex_half_t *>(k_f.data_ptr()),
            static_cast<complex_half_t *>(f_32_fft.data_ptr()),
