    ...
    uint signal_size)
{
  ...
  using BlockStore_Sequence = cub::BlockStore<c10::complex<float>, BLOCK_DIM_X, items_per_thread_input / 2, cub::BLOCK_STORE_STRIPED, BLOCK_DIM_Y>;
  ...
  // for the 32 x 32 dft
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::col_major> a_frag_dft_N_1[MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  
  // for 32 x 32 twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_32_dft_frag[MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  // for 32 x 32 twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_32_idft_frag[MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];

  // for the 32 x 1024 twiddle
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_1024_dft_frag[32 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  // for 32 x 1024 idft twiddle - split into 32 x (32 x 32)
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::col_major> twiddle_1024_idft_frag[32 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  ...
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> k_frag[32 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
    {
      ...
      wmma::load_matrix_sync(a_frag_dft_N_1[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + a_idx), sqrt_N_1);
      ...
      wmma::load_matrix_sync(a_frag_dft_N_1[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + a_idx), sqrt_N_1);
  ...
  for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_1024_dft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real) + k_idx_offset + b_idx), sqrt_N_1);
        wmma::load_matrix_sync(twiddle_1024_dft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag) + k_idx_offset + b_idx), sqrt_N_1);
      }
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_1 + k * WMMA_K : k * WMMA_K * sqrt_N_1 + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_32_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + b_idx), sqrt_N_1);
      wmma::load_matrix_sync(twiddle_32_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + b_idx), sqrt_N_1);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_1 + k * WMMA_K : k * WMMA_K * sqrt_N_1 + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_32_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real_2) + b_idx), sqrt_N_1);
      wmma::load_matrix_sync(twiddle_32_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + b_idx), sqrt_N_1);
    }
  }
  ...
  for (int h_tile_id = 0; h_tile_id < H_TILE_SIZE; h_tile_id++)
  {
    ...
    for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
    {
      ...
      for (int j_a = 0; j_a < MATMUL_WARP_WIDTH_1; j_a++)
      {
        ...
        for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
        {
          ...
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real + a_idx)), sqrt_N_1);
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag + a_idx)), sqrt_N_1);
        }
      }
    }
    ...
    for (int b_tile_id = 0; b_tile_id < B_TILE_SIZE; b_tile_id++)
    {
      ...
      //   scratch = __nv_bfloat162(a_input_data[2 * i].real(), a_input_data[2 * i + 1].real());
      //   reinterpret_cast<__nv_bfloat162 *>(a_real)[a_idx] = scratch;

      //   scratch = __nv_bfloat162(a_input_data[2 * i].imag(), a_input_data[2 * i + 1].imag());
      //   reinterpret_cast<__nv_bfloat162 *>(a_imag)[a_idx] = scratch;
      // }
      ...
      //    for (int i = 0; i < items_per_thread_input; i++) {
      //       a_idx = i * num_threads + thread_id;
      //       printf("%f, ", __bfloat162float(a_real[a_idx]));
      //    }
      //    printf("\n");
      // }
      ...
      // {
      //   a_idx = i * num_threads + thread_id;
      //   real = reinterpret_cast<__nv_bfloat162 *>(a_real)[a_idx];
      //   imag = reinterpret_cast<__nv_bfloat162 *>(a_imag)[a_idx];
      //   reinterpret_cast<c10::complex<____nv_bfloat16> *>(a_input_data)[2 * i] = c10::complex<____nv_bfloat16>(real.x, imag.x);
      //   reinterpret_cast<c10::complex<____nv_bfloat16> *>(a_input_data)[2 * i + 1] = c10::complex<____nv_bfloat16>(real.y, imag.y);
      // }
