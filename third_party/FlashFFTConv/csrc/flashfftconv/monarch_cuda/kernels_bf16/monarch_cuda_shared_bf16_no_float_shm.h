...
#include "shared/monarch_cuda_shared_bf16_matmuls.h"
#include "shared/monarch_cuda_shared_bf16_load_frags.h"
#if defined(__HIP_PLATFORM_AMD__)
...
{
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, ALayout> a_frag[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  load_a_frag_256<ALayout, a_trans, MATMUL_WARP_WIDTH, a_frag_from_acc>(a_real_inp, a_imag_inp, sqrt_N, N, acc_frag_half, a_frag);

  // __syncthreads();

  _complex_matmul_256<ALayout, BLayout, out_trans, MATMUL_WARP_WIDTH, output_to_shmem>(a_real_out, a_imag_out, sqrt_N, N, a_frag, b_frag, acc_frag_1, acc_frag_half, out_layout);
}
