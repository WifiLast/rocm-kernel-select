    ...
    uint sqrt_N)
{
  ...
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_K, WMMA_N, wmma_half_t> acc_frag_half[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
...
#pragma unroll
  ...
  for (int k_idx = 0; k_idx < 16 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_256_dft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real) + k_idx_offset + b_idx), sqrt_N);
        wmma::load_matrix_sync(twiddle_256_dft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag) + k_idx_offset + b_idx), sqrt_N);
      }
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_16_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + b_idx), sqrt_N);
      wmma::load_matrix_sync(twiddle_16_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + b_idx), sqrt_N);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_16_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real_2) + b_idx), sqrt_N);
      wmma::load_matrix_sync(twiddle_16_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + b_idx), sqrt_N);
    }
  }
    ...
    for (int k_idx = 0; k_idx < 16 / WARP_TILE_SIZE; k_idx++)
    {
      ...
      for (int j_a = 0; j_a < MATMUL_WARP_WIDTH; j_a++)
      {
        ...
        for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
        {
          ...
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real + a_idx)), sqrt_N);
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag + a_idx)), sqrt_N);
        }
      }
    }
