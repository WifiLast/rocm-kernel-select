    ...
    uint signal_size)
{
  ...
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_32_dft_frag[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  // for 32 x 32 twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_32_idft_frag[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];

  // for the 16 x 1024 twiddle
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_1024_dft_frag[16 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  // for 16 x 1024 idft twiddle - split into 64 x (16 x 16)
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::col_major> twiddle_1024_idft_frag[64 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  ...
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> k_frag[16 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  ...
  for (int k_idx = 0; k_idx < 16 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_1024_dft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real) + k_idx_offset + b_idx), sqrt_N_2);
        wmma::load_matrix_sync(twiddle_1024_dft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag) + k_idx_offset + b_idx), sqrt_N_2);
      }
    }
  }
  ...
  for (int k_idx = 0; k_idx < 64 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_1024_idft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real) + k_idx_offset + b_idx), 1024);
        wmma::load_matrix_sync(twiddle_1024_idft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag) + k_idx_offset + b_idx), 1024);
      }
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
    {
      ...
      wmma::load_matrix_sync(twiddle_32_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(twiddle_32_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_2 + k * WMMA_K : k * WMMA_K * sqrt_N_2 + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_32_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real_2) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(twiddle_32_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + b_idx), sqrt_N_2);
    }
  }
