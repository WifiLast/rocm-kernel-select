...
using wmma_half_t = rocwmma::bfloat16_t;
#else
using namespace nvcuda;
using wmma_half_t = __nv_bfloat16;
#endif
...
__global__ void monarch_conv_cuda_kernel(
    ...
    const c10::complex<at::BFloat16> *__restrict__ b_16,                        // 16 x 16
    const c10::complex<at::BFloat16> *__restrict__ twiddle_factors_N_fft,  // 8192
    ...
    const c10::complex<at::BFloat16> *__restrict__ b_16_ifft,                   // 16 x 16
    const c10::complex<at::BFloat16> *__restrict__ twiddle_factors_N_ifft, // 8192
    ...
    uint signal_size)
{
  ...
  at::BFloat16 *b_imag = &a_real[2 * N + N_1];
  at::BFloat16 *b_real_2 = &a_real[2 * N + 2 * N_1];
  at::BFloat16 *b_imag_2 = &a_real[2 * N + 3 * N_1];
  ...
  const int items_per_thread_matrix_N_2 = num_threads <= 128 ? N_2 / num_threads : 2;
  ...
  // for the 32 x 32 dft
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> b_frag_dft_N_1[MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  // for the 32 x 32 idft
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> b_frag_idft_N_1[MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];

  // for the 16 x 16 dft
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> b_frag_dft_N_2[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  // for the 16 x 16 idft
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> b_frag_idft_N_2[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  // for the 16 x 16 dft
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::col_major> a_frag_dft_N_2[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  
  // for 16 x 16 twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_16_dft_frag[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  // for 16 x 16 twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_16_idft_frag[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];

  // for the 32 x 256 twiddle
  wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_256_dft_frag[32 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  // for 32 x 256 idft twiddle
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::col_major> twiddle_256_idft_frag[8 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];

  // accumulator fragments for the 32 x 32 and 16 x 16
  ...
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_K, WMMA_N, wmma_half_t> acc_frag_1_half[MATMUL_WARP_WIDTH_1][MATMUL_WARP_WIDTH_1][2];
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_K, WMMA_N, wmma_half_t> acc_frag_2_half[MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];

  // for kernels - note that there are 32 / WARP_TILE_SIZE of these now!
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> k_frag[32 / WARP_TILE_SIZE][MATMUL_WARP_WIDTH_2][MATMUL_WARP_WIDTH_2][2];
  ...
  // start loading 16x16 ifft twiddle factors
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
    {
      ...
      wmma::load_matrix_sync(b_frag_dft_N_1[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + b_idx), sqrt_N_1);
      wmma::load_matrix_sync(b_frag_dft_N_1[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + b_idx), sqrt_N_1);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_1 + k * WMMA_K : k * WMMA_K * sqrt_N_1 + j_b * WMMA_N;
      wmma::load_matrix_sync(b_frag_idft_N_1[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real_2) + b_idx), sqrt_N_1);
      wmma::load_matrix_sync(b_frag_idft_N_1[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + b_idx), sqrt_N_1);
    }
  }

  // load N twiddle factors into registers
  // these will be loaded into the inner loop, so treat them as 32 x 256
  for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
      {
        ...
        wmma::load_matrix_sync(twiddle_256_dft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real) + k_idx_offset + b_idx), sqrt_N_2);
        wmma::load_matrix_sync(twiddle_256_dft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag) + k_idx_offset + b_idx), sqrt_N_2);
      }
    }
  }
  ...
  // load 16x16 twiddles into shared memory
  ...
  // start loading 16x16 DFT matrices
  // NOTE(danfu): this takes about 60 us
  BlockLoad_Matrix_N_2().Load(
  ...
  for (int k_idx = 0; k_idx < 8 / WARP_TILE_SIZE; k_idx++)
  {
    ...
    for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_1; j_b++)
    {
      ...
      for (int k = 0; k < MATMUL_WARP_WIDTH_1; k++)
      {
        b_idx = j_b * WMMA_N * 256 + k * WMMA_K;
        wmma::load_matrix_sync(twiddle_256_idft_frag[k_idx][k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real) + k_idx_offset + b_idx), 256);
        wmma::load_matrix_sync(twiddle_256_idft_frag[k_idx][k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag) + k_idx_offset + b_idx), 256);
      }
    }
  }

  // load 16x16 DFT twiddles into twiddle_dft_frag
  // #pragma unroll
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    // #pragma unroll
    for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
    {
      ...
      wmma::load_matrix_sync(twiddle_16_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(twiddle_16_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_2 + k * WMMA_K : k * WMMA_K * sqrt_N_2 + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_16_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real_2) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(twiddle_16_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + b_idx), sqrt_N_2);
    }
  }
  ...
  //   for (int i = 0; i < 32; i++) {
  //     a_idx = i * num_threads + thread_id;
  //     printf("%f + %fi, ", __bfloat162float(__nv_bfloat16(b_real[i])), __bfloat162float(__nv_bfloat16(b_imag[i])));
  //   }
  //   printf("\n");
  //   printf("b_16_ifft\n");
  //   for (int i = 0; i < 32; i++) {
  //     a_idx = i * num_threads + thread_id;
  //     printf("%f + %fi, ", __bfloat162float(__nv_bfloat16(b_real_2[i])), __bfloat162float(__nv_bfloat16(b_imag_2[i])));
  //   }
  //   printf("\n");
  // }

  // load the 16x16 DFT matrices into b_frag_dft_N_2, b_frag_idft_N_2
  #pragma unroll
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    // #pragma unroll
    for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
    {
      ...
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_2 + k * WMMA_K : k * WMMA_K * sqrt_N_2 + j_b * WMMA_N;
      wmma::load_matrix_sync(a_frag_dft_N_2[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + a_idx), sqrt_N_2);
      wmma::load_matrix_sync(b_frag_dft_N_2[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(a_frag_dft_N_2[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + a_idx), sqrt_N_2);
      wmma::load_matrix_sync(b_frag_dft_N_2[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH_2; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N_2 + k * WMMA_K : k * WMMA_K * sqrt_N_2 + j_b * WMMA_N;
      wmma::load_matrix_sync(b_frag_idft_N_2[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real_2) + b_idx), sqrt_N_2);
      wmma::load_matrix_sync(b_frag_idft_N_2[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + b_idx), sqrt_N_2);
    }
  }
  ...
  for (int h_tile_id = 0; h_tile_id < H_TILE_SIZE; h_tile_id++)
  {
    ...
    for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
    {
      ...
      for (int j_a = 0; j_a < MATMUL_WARP_WIDTH_2; j_a++)
      {
        ...
        for (int k = 0; k < MATMUL_WARP_WIDTH_2; k++)
        {
          ...
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_real + a_idx)), sqrt_N_2);
          wmma::load_matrix_sync(k_frag[k_idx][j_a][k][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(a_imag + a_idx)), sqrt_N_2);
        }
      }
    }
    ...
    if(out_gate != nullptr){
      ...
      for (int k_idx = 0; k_idx < 8 / WARP_TILE_SIZE; k_idx++)
      {
        ...
        // outer DFT
        complex_matmul_r2c_256<wmma::col_major, wmma::row_major, true, true, MATMUL_WARP_WIDTH_1, false, true>(
      ...
      for (int k_idx = 0; k_idx < 32 / WARP_TILE_SIZE; k_idx++)
      {
        ...
        // if (threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x == 0 && blockIdx.y == 0 && k_idx < 2) {
        //   printf("After first iDFT in the conv, %d\n", k_idx);
        //   for (int i = 0; i < 8; i++) {
        //     // a_idx = i * num_threads + thread_id + k_idx_offset;
        //     a_idx = i + k_idx_offset;
        //     printf("%f + %fi, ", __bfloat162float(__nv_bfloat16(a_real[a_idx])), __bfloat162float(__nv_bfloat16(a_imag[a_idx])));
        ...
        // if (threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.x == 0 && blockIdx.y == 0 && k_idx < 2) {
        //   printf("After second iDFT in the conv, %d\n", k_idx);
        //   for (int i = 0; i < 8; i++) {
        //     // a_idx = i * num_threads + thread_id + k_idx_offset;
        //     a_idx = i + k_idx_offset;
        //     printf("%f + %fi, ", __bfloat162float(__nv_bfloat16(a_real[a_idx])), __bfloat162float(__nv_bfloat16(a_imag[a_idx])));
        ...
        //   for (int i = 0; i < 8; i++) {
        //     a_idx = i * num_threads + thread_id + k_idx_offset;
        //     printf("%f + %fi, ", ____nv_bfloat162float(a_real[a_idx]), ____nv_bfloat162float(a_imag[a_idx]));
        //   }
        //   printf("\n");
        // }
      ...
      for (int k_idx = 0; k_idx < 8 / WARP_TILE_SIZE; k_idx++)
      {
        ...
        k_idx_offset = k_idx * WARP_TILE_SIZE * sqrt_N_1 + warp_id * sqrt_N_1;
        // outer DFT
        complex_matmul_c2r_256<wmma::col_major, wmma::row_major, true, true, MATMUL_WARP_WIDTH_1, false, true>(
