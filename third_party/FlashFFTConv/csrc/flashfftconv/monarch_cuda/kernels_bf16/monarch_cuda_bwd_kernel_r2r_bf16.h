    ...
    uint sqrt_N)
{
  ...
  using BlockStore_Sequence_Complex = cub::BlockStore<complex_bfloat16_t, BLOCK_DIM_X, items_per_thread_kf, cub::BLOCK_STORE_STRIPED, BLOCK_DIM_Y>;
  ...
  complex_bfloat16_t a_input_data[items_per_thread_input];    // for storing the input
  complex_bfloat16_t kf_input_data[items_per_thread_input];    // for storing the kf
  ...
  at::BFloat16 orig_input_data[items_per_thread_input];     // for storing the input
  at::BFloat16 ingate_data[items_per_thread_input];     // for storing the input
  at::BFloat16 outgate_data[items_per_thread_input];     // for storing the input
  at::BFloat16 dingate_data[items_per_thread_input];     // for storing the input
  at::BFloat16 doutgate_data[items_per_thread_input];     // for storing the input
  ...
  // for twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_dft_frag[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  // for twiddles
  wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_K, WMMA_N, wmma_half_t, wmma::row_major> twiddle_idft_frag[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  ...
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_K, WMMA_N, wmma_half_t> acc_frag_1_half[MATMUL_WARP_WIDTH][MATMUL_WARP_WIDTH][2];
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
    {
      ...
      wmma::load_matrix_sync(a_frag_dft[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + a_idx), sqrt_N);
      wmma::load_matrix_sync(b_frag_dft[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + b_idx), sqrt_N);
      wmma::load_matrix_sync(a_frag_dft[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + a_idx), sqrt_N);
      wmma::load_matrix_sync(b_frag_dft[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + b_idx), sqrt_N);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    for (int k = 0; k < MATMUL_WARP_WIDTH; k++)
    {
      ...
      wmma::load_matrix_sync(b_frag_idft[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real_2) + b_idx), sqrt_N);
      // wmma::load_matrix_sync(a_frag_idft[k][j_b][1], reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + a_idx, sqrt_N);
      wmma::load_matrix_sync(b_frag_idft[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + b_idx), sqrt_N);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_dft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real) + b_idx), sqrt_N);
      wmma::load_matrix_sync(twiddle_dft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag) + b_idx), sqrt_N);
    }
  }
  ...
  for (int j_b = 0; j_b < MATMUL_WARP_WIDTH; j_b++)
  {
    ...
    {
      b_idx = b_trans ? j_b * WMMA_N * sqrt_N + k * WMMA_K : k * WMMA_K * sqrt_N + j_b * WMMA_N;
      wmma::load_matrix_sync(twiddle_idft_frag[k][j_b][0],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_real_2) + b_idx), sqrt_N);
      wmma::load_matrix_sync(twiddle_idft_frag[k][j_b][1],reinterpret_cast<const wmma_half_t*>( reinterpret_cast<__nv_bfloat16 *>(b_imag_2) + b_idx), sqrt_N);
    }
  }
  ...
  for (int h_tile_id = 0; h_tile_id < H_TILE_SIZE; h_tile_id++)
  {
    ...
    BlockLoad_Filter().Load(
        ...
        reinterpret_cast<complex_bfloat16_t(&)[items_per_thread_kf]>(kf_input_data));

    if (thread_id == 0)
    {
      // load in the pivot into the imag position
      kf_input_data[0] = complex_bfloat16_t(kf_input_data[0].real(), (k_f + h_offset_kernel + h_tile_id * (N + 1))[N].real());
    }
    ...
    for (int b_tile_id = 0; b_tile_id < B_TILE_SIZE; b_tile_id++)
    {
      ...
      if (in_gate != nullptr) {
        ...
        for (int i = 0; i < items_per_thread_input / 2; i++) {
          reinterpret_cast<__nv_bfloat162 *>(orig_input_data)[i] = reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i];
          reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i] = __hmul2(
            reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i],
            reinterpret_cast<__nv_bfloat162 *>(ingate_data)[i]
          );
        }
      }
      ...
      if (out_gate != nullptr) { // compute dout_gate
        ...
        for (int i = 0; i < items_per_thread_input / 2; i++) {
          reinterpret_cast<__nv_bfloat162 *>(doutgate_data)[i] = __hmul2(
            reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i],
            reinterpret_cast<__nv_bfloat162 *>(doutgate_data)[i]
          );
        }
        ...
        for (int i = 0; i < items_per_thread_input / 2; i++) {
          reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i] = __hmul2(
            reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i],
            reinterpret_cast<__nv_bfloat162 *>(outgate_data)[i]
          );
        }
      ...
      for (int i = 0; i < items_per_thread_kf; i++)
      {
        reinterpret_cast<__nv_bfloat162 *>(a_input_data)[i] = __hmul2(
          reinterpret_cast<__nv_bfloat162 *>(a_input_data)[i],
          __nv_bfloat162(
            __float2bfloat16(float(N)),
            __float2bfloat16(float(N))
          )
        );
      }
      ...
      if (thread_id == 0) {
        reinterpret_cast<__nv_bfloat162 *>(a_input_data)[0] = __hmul2(
          __nv_bfloat162(
            __nv_bfloat16(a_input_data[0].real()),
            __nv_bfloat16(a_input_data[0].imag())
          ),
          __nv_bfloat162(
            __float2bfloat16(0.5),
            __float2bfloat16(0.5)
          )
        );
      }
      ...
      if (in_gate != nullptr) {
        ...
        for (int i = 0; i < items_per_thread_input / 2; i++) {
          reinterpret_cast<__nv_bfloat162 *>(dingate_data)[i] = __hmul2(
            reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i],
            reinterpret_cast<__nv_bfloat162 *>(orig_input_data)[i]
          );
          reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i] = __hmul2(
            reinterpret_cast<__nv_bfloat162 *>(x_input_data)[i],
            reinterpret_cast<__nv_bfloat162 *>(ingate_data)[i]
    ...
    if (thread_id == 0) {
      complex_bfloat16_t pivot = complex_bfloat16_t(temp[0].imag(), 0.);
      temp[0] = complex_bfloat16_t(temp[0].real(), 0.);
      (dk_f_out + h_offset_kernel + blockIdx.x * H * (N + 1) + h_tile_id * (N+1))[N] = pivot;
    }

    // store dk_f
    BlockStore_Sequence_Complex().Store(
        reinterpret_cast<complex_bfloat16_t *>(dk_f_out + h_offset_kernel + blockIdx.x * H * (N + 1) + h_tile_id * (N+1)),
        reinterpret_cast<complex_bfloat16_t(&)[items_per_thread_kf]>(temp));
  }   // h_tile_id
}