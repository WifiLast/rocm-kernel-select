...
#include "kernels_fp16/monarch_cuda_bwd_kernel.h"
#include "kernels_fp16/monarch_cuda_16_16_16_bwd_kernel.h"
#include "kernels_fp16/monarch_cuda_32_16_16_bwd_kernel.h"
#include "kernels_fp16/monarch_cuda_16_32_32_bwd_kernel.h"
#include "kernels_fp16/monarch_cuda_32_32_32_bwd_kernel.h"
#if defined(__HIP_PLATFORM_AMD__)
...
#define CHECK_LAST_CUDA_ERROR() checkLastFP16Bwd(__FILE__, __LINE__)
void checkLastFP16Bwd(const char* const file, const int line)
{
...
std::vector<torch::Tensor> monarch_conv_bwd_cuda(
  torch::Tensor dout,
  torch::Tensor x,
  torch::Tensor k_f,
    ...
    case 256:
      if (B >= 2 && (B % 8) == 0 && (H % 4) == 0) {
        ...
        monarch_conv_bwd_cuda_kernel<32, 1, 256, 1, false, 2, 4><<<gridDim, blockDim, 8 * fftsize * sizeof(half)>>>(
            static_cast<at::Half *>(dout.data_ptr()),
            ...
            static_cast<at::Half *>(dx_out.data_ptr()),
            ...
            in_gate.has_value() ? static_cast<at::Half *>(din_gate.data_ptr()) : nullptr,
            out_gate.has_value() ? static_cast<at::Half *>(dout_gate.data_ptr()) : nullptr,
            B,
            H,
            N,
            sqrt_N);
...
std::vector<torch::Tensor> monarch_conv_bwd_cuda_16_16_16(
   torch::Tensor dout,
   torch::Tensor x,
   torch::Tensor k_f,
   torch::Tensor f_16_fft,
  ...
  if(out_gate.has_value()){
    dout_gate = torch::empty_like(out_gate.value());
    out = monarch_conv_cuda_16_16_16(
      x,
      k_f,
      f_16_fft,
...
std::vector<torch::Tensor> monarch_conv_bwd_cuda_32_16_16(
  torch::Tensor dout,
  torch::Tensor x,
  torch::Tensor k_f,
  torch::Tensor f_32_fft,
  ...
  if(out_gate.has_value()){
    dout_gate = torch::empty_like(out_gate.value());
    out = monarch_conv_cuda_32_16_16(
      x,
      k_f,
      f_32_fft,
      f_16_fft,
...
std::vector<torch::Tensor> monarch_conv_bwd_cuda_16_32_32(
  torch::Tensor dout,
  torch::Tensor x,
  torch::Tensor k_f,
  torch::Tensor f_16_fft,
  ...
  if(out_gate.has_value()){
    dout_gate = torch::empty_like(out_gate.value());
    out = monarch_conv_cuda_16_32_32(
      x,
      k_f,
      f_16_fft,
      f_32_fft,
...
std::vector<torch::Tensor> monarch_conv_bwd_cuda_32_32_32(
  torch::Tensor dout,
  torch::Tensor x,
  torch::Tensor k_f,
  torch::Tensor f_32_fft,
  ...
  if(out_gate.has_value()){
    dout_gate = torch::empty_like(out_gate.value());
    out = monarch_conv_cuda_32_32_32(
      x,
      k_f,
      f_32_fft,
      twiddle_factors_N_fft,
