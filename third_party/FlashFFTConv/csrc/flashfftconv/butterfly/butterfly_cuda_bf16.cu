    ...
    int N)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        wmma::load_matrix_sync(a_frag_real[i], reinterpret_cast<wmma_half_t*>(d_f_real_shared + i * N * 16 + threadIdx.y * 16), N);
        wmma::load_matrix_sync(a_frag_imag[i], reinterpret_cast<wmma_half_t*>(d_f_imag_shared + i * N * 16 + threadIdx.y * 16), N);
    ...
    int N)
{
    ...
    if (threadIdx.y < 4)
    {
        ...
        wmma::load_matrix_sync(a_frag_real, reinterpret_cast<wmma_half_t*>(d_f_real_shared), N);
        wmma::load_matrix_sync(a_frag_imag, reinterpret_cast<wmma_half_t*>(d_f_imag_shared), N);
