...
#pragma unroll
        for (int j = 0; j < 4; j++)
        {
            wmma::load_matrix_sync(a_frag_real[i][j], reinterpret_cast<wmma_half_t*>(d_f_real_shared + j * N * 16 + i * 16), N);
            wmma::load_matrix_sync(a_frag_imag[i][j], reinterpret_cast<wmma_half_t*>(d_f_imag_shared + j * N * 16 + i * 16), N);
        }
