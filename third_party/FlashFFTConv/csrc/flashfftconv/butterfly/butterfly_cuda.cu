    ...
    int N)
{
    ...
    wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::col_major> a_frag_real[4];
    wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::row_major> tw_frag_real[4];
    wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::row_major> tw_frag_imag[4];
    wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::col_major> a_frag_imag[4];
    wmma::fragment<wmma::matrix_b, 16, 16, 16, wmma_half_t, wmma::row_major> b_frag[4][4];
    wmma::fragment<wmma::accumulator, 16, 16, 16, wmma_half_t> acc_frag_real[4];
    wmma::fragment<wmma::accumulator, 16, 16, 16, wmma_half_t> acc_frag_imag[4];

    __syncthreads();

    for (int i = 0; i < 4; i++)
    {
        wmma::load_matrix_sync(a_frag_real[i], reinterpret_cast<wmma_half_t*>(d_f_real + i * N * 16 + threadIdx.y * 16), N);
        wmma::load_matrix_sync(a_frag_imag[i], reinterpret_cast<wmma_half_t*>(d_f_imag + i * N * 16 + threadIdx.y * 16), N);
        wmma::load_matrix_sync(tw_frag_real[i], reinterpret_cast<wmma_half_t*>(twiddles_real_shared + threadIdx.y * N * 16 + i * 16), N);
        wmma::load_matrix_sync(tw_frag_imag[i], reinterpret_cast<wmma_half_t*>(twiddles_imag_shared + threadIdx.y * N * 16 + i * 16), N);
    }
    ...
    for (int t = 0; t < 16; t++)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                wmma::load_matrix_sync(b_frag[i][j], reinterpret_cast<wmma_half_t*>(x_shared + i * N * 16 + j * 16), N);
            }
        }
...
#pragma unroll
        for (int j = 0; j < 4; j++)
        {
            ...
            wmma::store_matrix_sync(reinterpret_cast<wmma_half_t*>(out_real_shared + threadIdx.y * N * 16 + j * 16), acc_frag_real[j], N, wmma::mem_row_major);
            wmma::store_matrix_sync(reinterpret_cast<wmma_half_t*>(out_imag_shared + threadIdx.y * N * 16 + j * 16), acc_frag_imag[j], N, wmma::mem_row_major);
        }
    ...
    int N)
{
    ...
    if (threadIdx.y < N / 16)
    {
        ...
        wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::col_major> a_frag_real[2][2];
        wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::row_major> tw_frag_real[2][2];
        wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::row_major> tw_frag_imag[2][2];
        wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::col_major> a_frag_imag[2][2];
        wmma::fragment<wmma::matrix_b, 16, 16, 16, wmma_half_t, wmma::row_major> b_frag[2][2];
        wmma::fragment<wmma::accumulator, 16, 16, 16, wmma_half_t> acc_frag_real[2][2];
        wmma::fragment<wmma::accumulator, 16, 16, 16, wmma_half_t> acc_frag_imag[2][2];
        ...
        for (int i = 0; i < 2; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                wmma::load_matrix_sync(a_frag_real[i][j], reinterpret_cast<wmma_half_t*>(d_f_real + j * N * 16 + i * 16), N);
                wmma::load_matrix_sync(a_frag_imag[i][j], reinterpret_cast<wmma_half_t*>(d_f_imag + j * N * 16 + i * 16), N);
                wmma::load_matrix_sync(b_frag[i][j], reinterpret_cast<wmma_half_t*>(x_shared + i * 2 * N * 16 + j * 16 + t), 2 * N);
                wmma::load_matrix_sync(tw_frag_real[i][j], reinterpret_cast<wmma_half_t*>(twiddles_real_shared + i * 2 * N * 16 + j * 16 + t), 2 * N);
                wmma::load_matrix_sync(tw_frag_imag[i][j], reinterpret_cast<wmma_half_t*>(twiddles_imag_shared + i * 2 * N * 16 + j * 16 + t), 2 * N);
            }
        }
...
#pragma unroll
        for (int i = 0; i < 2; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                ...
                wmma::store_matrix_sync(reinterpret_cast<wmma_half_t*>(out_real_shared + i * 2 * N * 16 + j * 16 + t), acc_frag_real[i][j], 2 * N, wmma::mem_row_major);
                wmma::store_matrix_sync(reinterpret_cast<wmma_half_t*>(out_imag_shared + i * 2 * N * 16 + j * 16 + t), acc_frag_imag[i][j], 2 * N, wmma::mem_row_major);
            }
        }
    }
    ...
    int N)
{
    ...
    wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::col_major> a_frag_real[8];
    wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::row_major> tw_frag_real[8];
    wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::row_major> tw_frag_imag[8];
    wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::col_major> a_frag_imag[8];
    wmma::fragment<wmma::matrix_b, 16, 16, 16, wmma_half_t, wmma::row_major> b_frag[8][8];
    wmma::fragment<wmma::accumulator, 16, 16, 16, wmma_half_t> acc_frag_real[8];
    wmma::fragment<wmma::accumulator, 16, 16, 16, wmma_half_t> acc_frag_imag[8];
    ...
    for (int i = 0; i < 8; i++){
        wmma::load_matrix_sync(a_frag_real[i], reinterpret_cast<wmma_half_t*>(shared_real + i * 128 * 16 + threadIdx.y * 16), 128);
        wmma::load_matrix_sync(a_frag_imag[i], reinterpret_cast<wmma_half_t*>(shared_imag + i * 128 * 16 + threadIdx.y * 16), 128);
    }
    ...
    for (int i = 0; i < 8; i++){
        wmma::load_matrix_sync(tw_frag_real[i], reinterpret_cast<wmma_half_t*>(shared_real + threadIdx.y * 128 * 16 + i * 16), 128);
        wmma::load_matrix_sync(tw_frag_imag[i], reinterpret_cast<wmma_half_t*>(shared_imag + threadIdx.y * 128 * 16 + i * 16), 128);
    }
    ...
    for(int t=0; t< 16; t++){
        ...
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < 8; j++)
            {
                wmma::load_matrix_sync(b_frag[i][j], reinterpret_cast<wmma_half_t*>(shared_real + i * 128 * 16 + j * 16), 128);
            }
        }
    ...
    #pragma unroll
            for (int j = 0; j < 8; j++)
            {
                ...
                wmma::store_matrix_sync(reinterpret_cast<wmma_half_t*>(shared_real + threadIdx.y * 128 * 16 + j * 16), acc_frag_real[j], 128, wmma::mem_row_major);
                wmma::store_matrix_sync(reinterpret_cast<wmma_half_t*>(shared_imag + threadIdx.y * 128 * 16 + j * 16), acc_frag_imag[j], 128, wmma::mem_row_major);
            }
    ...
    int N)
{
    ...
    if (threadIdx.y < 4)
    {
        ...
        wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::col_major> a_frag_real;
        wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::row_major> tw_frag_real;
        wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::row_major> tw_frag_imag;
        wmma::fragment<wmma::matrix_a, 16, 16, 16, wmma_half_t, wmma::col_major> a_frag_imag;
        wmma::fragment<wmma::matrix_b, 16, 16, 16, wmma_half_t, wmma::row_major> b_frag;
        wmma::fragment<wmma::accumulator, 16, 16, 16, wmma_half_t> acc_frag_real;
        wmma::fragment<wmma::accumulator, 16, 16, 16, wmma_half_t> acc_frag_imag;

        wmma::load_matrix_sync(a_frag_real, reinterpret_cast<wmma_half_t*>(d_f_real), N);
        wmma::load_matrix_sync(a_frag_imag, reinterpret_cast<wmma_half_t*>(d_f_imag), N);
        wmma::load_matrix_sync(b_frag, reinterpret_cast<wmma_half_t*>(x_shared + threadIdx.y * 16), 64);
        wmma::load_matrix_sync(tw_frag_real, reinterpret_cast<wmma_half_t*>(twiddles_real_shared + threadIdx.y * 16), 64);
        wmma::load_matrix_sync(tw_frag_imag, reinterpret_cast<wmma_half_t*>(twiddles_imag_shared + threadIdx.y * 16), 64);
        ...
        wmma::store_matrix_sync(reinterpret_cast<wmma_half_t*>(out_real_shared + threadIdx.y * 16), acc_frag_real, 64, wmma::mem_row_major);
        wmma::store_matrix_sync(reinterpret_cast<wmma_half_t*>(out_imag_shared + threadIdx.y * 16), acc_frag_imag, 64, wmma::mem_row_major);
    }
