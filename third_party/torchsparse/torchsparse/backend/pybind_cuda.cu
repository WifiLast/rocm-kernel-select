...
// These implicit-GEMM / fetch-on-demand dataflows are implemented with
// NVIDIA-only tensor-core intrinsics (wmma / mma.sync / nvcuda::) that have
// no HIP/rocWMMA-compatible port here, so their .cu sources are excluded
// from the HIP build (see setup.py). Guard the declarations and the
// corresponding m.def(...) registrations below so the extension still
// links under HIP; GatherScatter remains available as the ROCm conv backend.
