...
// CUDA has had a packed atomicAdd(__half2 *, __half2) since sm_60. HIP's
// amd_hip_atomic.h has no __half2 overload at all, so emulate it with a CAS
// loop on the 32-bit word a __half2 occupies. Kept as a named helper rather
// than an atomicAdd() overload so a future ROCm gaining the real overload
// cannot make the call site ambiguous.
#ifdef __HIP_PLATFORM_AMD__
__device__ __forceinline__ half2 atomic_add_half2(half2 *address, half2 val) {
  unsigned int *as_uint = reinterpret_cast<unsigned int *>(address);
  unsigned int old = *as_uint, assumed;
  do {
    assumed = old;
    half2 sum = __hadd2(*reinterpret_cast<half2 *>(&assumed), val);
    old = atomicCAS(as_uint, assumed, *reinterpret_cast<unsigned int *>(&sum));
  } while (assumed != old);
...
__global__ void scatter_all_kernel_pad_sep(
  ...
  for (int k = 0; k < kernel_volume; k++) {
    ...
    if (isfloat) {
      ...
      atomic_add_half2(
          &CONVERT_HALF2(out_feat[out_pos * c + (j << 1)]),
...
at::Tensor conv_forward_gather_scatter_cuda_latest(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      in_feat.scalar_type(), "conv_forward_gather_scatter_cuda", ([&] {
        // Elements handled per block: 256 threads x 4 (fp32) or x 8 (fp16),
        // matching `offset` in the kernel -- so 256 << 2 or 256 << 3. The
        // parentheses are the precedence the original relied on implicitly.
        // Hoisted out of the <<<>>> because hipify mis-parses a `<<` shift
        // inside a launch configuration and emits two spurious arguments.
        const int elems_per_block = 256 << ((sizeof(scalar_t) == 2) + 2);
        const int n_gather_blocks = (int)ceil(
            (double)(n_in_feats * n_in_channels) / elems_per_block);
