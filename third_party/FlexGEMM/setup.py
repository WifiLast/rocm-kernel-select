...
# NOTE: deliberately no -std= flag below. torch/all.h hard-#errors with
# "C++20 or later compatible compiler is required to use PyTorch" on torch
# >= 2.9, so a pinned -std=c++17 breaks the build against a current torch.
# BuildExtension appends the standard its own headers require (see
# append_std17_if_no_std_present() in torch/utils/cpp_extension.py) to every
# flag list that does not already carry a -std=/std: of its own, so omitting
# it here tracks whatever torch is installed instead of guessing.
if platform.system() == "Windows":
    extra_compile_args = {
        "cxx": ["/O2", "/EHsc", "/openmp", "/permissive-", "/Zc:__cplusplus"],
        "nvcc": ["-O3", "-Xcompiler=/EHsc", "-Xcompiler=/permissive-", "-Xcompiler=/Zc:__cplusplus"] + cc_flag,
    }
else:
    # Match PyTorch's CXX11 ABI setting
    cxx11_abi = "1" if torch.compiled_with_cxx11_abi() else "0"
    extra_compile_args = {
        "cxx": ["-O3", "-fopenmp", f"-D_GLIBCXX_USE_CXX11_ABI={cxx11_abi}"],
        "nvcc": ["-O3"] + cc_flag,
    }
