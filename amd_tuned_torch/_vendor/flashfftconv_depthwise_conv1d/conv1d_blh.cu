...
// (csrc/flashfftconv/conv1d/conv1d_blh.cu), Apache License 2.0 -- see LICENSE in
...
{
    // Upstream used __float2bfloat162_rn(0.0f) (broadcast a scalar); HIP has
    // no such overload, and hipify does not map the name, so spell the same
    // zero via __float22bfloat162_rn, which exists identically on both.
    return l + k < p || l + k > L_eff - (p + 1) ? __float22bfloat162_rn(make_float2(0.0f, 0.0f)) : u[b * L * D + (l + k - p) * D + d];
}
...
// Upstream declares this `T` but never returns anything (it writes through
// `out` instead, and the caller discards the value) -- amdclang warns
// -Wreturn-type and then, at -O3, treats falling off the end as unreachable
// and deletes the whole body, so the k == 3 path silently produced all
// zeros on ROCm. `void` is what the body actually is.
template<typename T, typename U>
__forceinline__ __device__ void _conv1d_k_3(const T* u, const U* weights, const U* bias, T* out, uint padding, uint b, uint l, uint d, uint t, uint L, uint D, uint K, uint L_eff, uint L_out)
{
