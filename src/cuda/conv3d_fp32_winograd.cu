// =================================================================================
// conv3d_fp32_winograd.cu -- F(2x2x2, 3x3x3) 3D Winograd conv3d for amd_tuned_torch
// on ROCm/RDNA3 (RX 7900 XTX, gfx1100). Ported from the original CMP-Turing
// project's CUDA kernel (src/cuda/kernel_example/fp32_conv3d_winograd.cu) --
// mechanical HIP translation only (hip/hip_runtime.h, hipMalloc/hipFree/
// hipGetLastError/hipSuccess, hipLaunchKernelGGL instead of the <<<>>>
// launch syntax, and a hipStream_t threaded through every launch so this
// runs on PyTorch's current stream instead of the default/null stream --
// same convention as conv3d_fp32.cu/conv3d_fp16.cu). The transform math
// itself is untouched.
//
// Alternative to conv3d_fp32.cu's direct kernel for the specific case this
// actually applies to: fp32, batch=1, kernel=3x3x3, stride=1, padding=1,
// dilation=1, groups=1. Falls back (returns false, `output` untouched) for
// anything outside that scope -- see launch_conv3d_fp32_winograd's guard
// block, checked FIRST, before any allocation. src/main_rocm.cpp's
// custom_conv3d_forward is the actual caller: it benchmarks this against
// conv3d_fp32.cu's direct kernel once per distinct shape and caches
// whichever wins, never assuming Winograd is faster just because it's
// applicable (see run_conv3d_fp32 there).
//
...
// float32 rounding only) before ever being benchmarked, on the original
// CUDA/Turing build. Not yet re-verified numerically after this HIP port --
// see tests_hardware/README.md before trusting it on real RDNA3 hardware.
//
...
// block, not one) -- this is NOT optional/cosmetic, per the original
// CUDA-side measurement this was ported from: an unblocked version (one
// block per single (tile,cout) pair) measured 147.7ms at the reference
// HunyuanVideo VAE-decode shape (512ch, 8x32x32) -- slower than the direct
// kernel's 31.9ms despite doing ~5.6x fewer multiplies in the accumulate
// step, because every block re-read the ENTIRE U[cout] row from global
// memory once per tile (redundant across all 1024 tiles sharing that cout)
// and the entire V[tile] row once per cout (redundant across all 512
// couts sharing that tile). BT_TILES=BT_COUT=8 register blocking (classic
// GEMM-style: each thread loads 8 U-values + 8 V-values per Cin step and
// reuses them across 64 partial products instead of 1) cut that to
// 19.6-20.2ms on the original build -- these are CUDA/Turing numbers, not
// RDNA3 ones; re-bench via tools/bench.py once built here rather than
// trusting them to transfer. Sizes above and below 8x8 measured worse on
// the original hardware (4x4: register blocking helps but not enough;
// 16x8/16x16: more register pressure costs more in occupancy than it saves
// in memory traffic) -- 8x8 is carried over as the starting point, not
// re-tuned for RDNA3's different register file/occupancy characteristics.
//
...
// for the reference shape -- 1024 tiles, 512 channels -- not enforced to
// be true in general); batch=1 only (this kernel has no batch dimension at
// all, unlike conv3d_fp32.cu). Any of these failing, or any other dtype,
// falls straight back to conv3d_fp32.cu.
//
...
// The original CUDA build relied on nvcc's --fmad=false (a project-wide
// setup.py flag) to keep this literal; there is no HIP/hipcc equivalent
// wired into this project's setup.py (hipcc's analogous flag is
// -ffp-contract=off, clang-based) -- FMA contraction may still happen at
// the compiler's discretion on this build. That only affects rounding by
// an ULP or two (FMA is strictly more precise per contraction, never
// less), not correctness, so this is a precision-reproducibility note, not
// a known bug.
// =================================================================================
...
// bounds), separable-transforms it to V.
// ---------------------------------------------------------------------
...
// header for why BT_TILES=BT_COUT=8 specifically and how much this
// blocking matters. Block = 64 threads; grid =
    ...
    int D_out, int H_out, int W_out)
{
    ...
    // WINOGRAD_BT_TILES*WINOGRAD_BT_COUT ever exceeds 64 (an earlier
    // `if`-based version on the original CUDA build silently dropped every
...
// D/H/W, divisible tile-count/C_out) is checked here. Runs on `stream`
// (PyTorch's current HIP stream), not the null stream, so this composes
// correctly with the rest of a captured/streamed graph.
// ---------------------------------------------------------------------
    ...
    hipStream_t stream)
{
    ...
    if (hipMalloc(&U, (size_t)C_out * C_in * 64 * sizeof(float)) != hipSuccess) return false;
    if (hipMalloc(&V, (size_t)C_in * total_tiles * 64 * sizeof(float)) != hipSuccess) {
...

    {
        ...
        hipLaunchKernelGGL(winograd_filter_transform_fp32, dim3(grid), dim3(block), 0, stream,
                            weight, U, C_out, C_in);
    }
    {
        ...
        hipLaunchKernelGGL(winograd_input_transform_fp32, dim3((unsigned)grid), dim3(block), 0, stream,
    ...
    }
    {
        ...
        hipLaunchKernelGGL(winograd_accumulate_output_fp32, grid, dim3(64), 0, stream,
                            U, V, bias, output, C_out, C_in, tiles_d, tiles_h, tiles_w,
                            D_out, H_out, W_out);
    }
