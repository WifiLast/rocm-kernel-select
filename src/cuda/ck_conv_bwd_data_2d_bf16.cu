// CK WMMA grouped-conv backward-data instantiation: 2D, ck::bhalf_t.
...
int ck_count_bwd_data_bf16() { return detail::count_bwd_data<ck::bhalf_t>(); }

bool ck_entry_bwd_data_bf16(int idx, int action, const ConvProblem& p, const void* p_grad_out,
                            const void* p_wei, void* p_grad_in, void* p_ws, size_t* out_ws,
                            hipStream_t stream) {
    return detail::entry_bwd_data<ck::bhalf_t>(idx, static_cast<detail::Action>(action), p,
