// CK WMMA grouped-conv backward-weight instantiation: 2D, ck::bhalf_t.
...
int ck_count_bwd_weight_bf16() { return detail::count_bwd_weight<ck::bhalf_t>(); }

bool ck_entry_bwd_weight_bf16(int idx, int action, const ConvProblem& p, const void* p_in,
                              const void* p_grad_out, void* p_grad_wei, void* p_ws, size_t* out_ws,
                              hipStream_t stream) {
    return detail::entry_bwd_weight<ck::bhalf_t>(idx, static_cast<detail::Action>(action), p, p_in,
